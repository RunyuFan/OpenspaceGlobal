...
class Global_Local_block(nn.Module):
    def __init__(self, dim):
        super().__init__()
        # assert model_name in mit_settings.keys(), f"MiT model name should be in {list(mit_settings.keys())}"
        self.dim = dim
        embed_dims, depths = [[self.dim], [1]]
        drop_path_rate = 0.1
        self.embed_dims = embed_dims
        # self.in_channels = in_channels
        ...
        cur = 0
        self.block1 = nn.ModuleList([Block(embed_dims[0], 8, 8, dpr[cur+i]) for i in range(depths[0])])  # dim, head, sr_ratio=1, dpr=0.
        self.norm1 = nn.LayerNorm(embed_dims[0])
...
if __name__ == '__main__':
    model = Global_Local_block(64)
    x = torch.zeros(1, 3, 256, 256)
    print('model2 parameters:', sum(p.numel() for p in model.parameters()))
